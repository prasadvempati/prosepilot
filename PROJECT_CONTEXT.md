...
## Real root cause of the recurring "Could not find text node containing" warnings: a race condition (2026-08-10, v1.0.12)

After the MISSING LINKING VERB rule above went live, the user retested and the same "Could not find text node containing" warning appeared again — for "My book on the table." (the new rule's full-sentence match) and separately for "book" alone. On inspection this was **not** another instance of the cross-node DOM-structure bug that v1.0.10/v1.0.11 addressed. `triggerCheck()`'s `await checkText(text)` call has no staleness check before rendering: the DeepSeek/LanguageTool round trip (0.5-2+ seconds) routinely takes longer than the 300ms debounce window, so when the user types steadily, an older/slower in-flight check can resolve *after* a newer check has already run and mutated the DOM (e.g. auto-rendering an earlier accepted "book"→"Book" capitalization fix). By the time the older check tries to render "book" (lowercase), it's genuinely no longer in the DOM — the warning was accurate, not a bug in the underline-matching logic itself.

Added a staleness guard in `triggerCheck()`: immediately after `checkText()` resolves, re-read the element's current text via `getElementText(el)` and compare it to the `text` that was actually sent for this check. If they no longer match, discard the result entirely (log and return) rather than attempting to render it — a newer check already superseded it, so nothing is lost. This is a general correctness fix, not Outlook-specific; it protects every site the extension runs on against the same class of race whenever a check happens to take longer than the user's next typing pause.

Verified via `node --check`. Bumped `manifest.json` to **v1.0.12**. Same distribution caveat as v1.0.10/v1.0.11 — reload from `edge://extensions` to pick up locally; still needs a separate Chrome Web Store submission once v1.0.9's review resolves.
